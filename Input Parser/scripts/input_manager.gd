...
#	Lookup and store D-Pad input and L.Stick input as 0 through 8.
	#TODO: Refactor into single function that iterates D-Pad, L.Stick, R.Stick, and NESW Buttons.
			#Separate and simultaneously broadcast all as signals. 
	...
	if left_angle != prev_left_angle or right_angle != prev_right_angle:
		SignalBus.inputs_changed.emit(left_angle, right_angle)

# Backup, Print output.
#	TODO: Move diagnostic output to Panel Container Node, and receive signals.
	#if left_angle != prev_left_angle or right_angle != prev_right_angle:
		#%CurrentInput/LeftArrow.text = str(ARROW_LOOKUP[left_angle])
		#%CurrentInput/LeftInput.text = str(left_angle)
		#%CurrentInput/RightInput.text = str(right_angle)
		#%CurrentInput/RightArrow.text = str(ARROW_LOOKUP[right_angle])
		#
		#prints (ARROW_LOOKUP[left_angle], left_angle, ARROW_LOOKUP[right_angle], right_angle,
		#left_stick_vector, right_stick_vector
		#)
